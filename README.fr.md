...
Il est agnostique du modèle et extensible, basé sur des 'stratégies' et des 'adaptateurs'.

Les stratégies de chat offrent un moyen de modifier dynamiquement le contexte, les messages système et les outils disponibles à chaque exécution pour optimiser pour une tâche ou un environnement d'ingénierie particulier.
...
2.  `backendStrategy` - un exemple légèrement plus complet où le LLM travaille sur une application Fastify locale (en cours d'exécution sur http://localhost:8080) pour créer des migrations de base de données et des points de terminaison API
...
2. Copiez `.env.example` en `.env` et ajoutez au moins l'une des clés `OPENAI_API_KEY` ou `ANTHROPIC_API_KEY`
3. Exécutez `bin/cli`
...
Les demandes de tirage sont bienvenues. Pour les modifications majeures, veuillez d'abord ouvrir un problème pour discuter de ce que vous aimeriez modifier.
