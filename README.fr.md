...
2. `backendStrategy` - un exemple légèrement plus complet où le LLM travaille sur une application Fastify locale (s'exécutant sur http://localhost:8080) pour créer des migrations de base de données et des points de terminaison API
3. `shellStrategy` - un shell alimenté par LLM qui peut écrire des fichiers et exécuter des processus

Les adaptateurs rendent tout LLM fondamental (GPT, Claude) interchangeables à chaud.
...
Les pull requests sont les bienvenus. Pour les changements majeurs, veuillez d'abord ouvrir une issue pour discuter de ce que vous aimeriez modifier.
