...
Ce projet comprend 3 exemples de stratégies :

1.  `demoStrategy` - un exemple simple et illustratif qui sert de point de départ pour créer de nouvelles stratégies
2.  `backendStrategy` - un exemple un peu plus complet où le LLM travaille sur une application Fastify locale (s'exécutant sur http://localhost:8080) pour créer des migrations de base de données et des points de terminaison API
3.  `shellStrategy` - un shell alimenté par LLM qui peut écrire des fichiers et exécuter des processus

Les adaptateurs permettent de remplacer facilement n'importe quel LLM fondamental (GPT, Claude).

## Pour commencer

1. Assurez-vous que Docker est installé et en cours d'exécution
2. Copiez `.env.example` vers `.env` et ajoutez au moins une clé parmi `OPENAI_API_KEY` ou `ANTHROPIC_API_KEY`
3. Exécutez `bin/cli`
